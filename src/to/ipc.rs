...
mod tests {
    ...
    fn test_write_valid_arrow() {
        ...
        let writer = IpcWriterImpl::default();
        writer.write_data(file_path, &df, false).unwrap();

        // Read the Parquet file back
        let read_df = IpcReader::new(File::open(file_path).unwrap())
            .finish()
            .unwrap();
    ...
    #[test]
    fn test_write_empty_dataframe_to_arrow() {
        // Create a temporary file for the Parquet output
        let temp_file = NamedTempFile::new().unwrap();
        let file_path = temp_file.path().to_str().unwrap();
    ...
    #[test]
    fn test_write_arrow_to_nonexistent_directory() {
    ...
    #[test]
    fn test_write_overwrite_arrow_file() {
        // Create a temporary file for the Parquet output
        let temp_file = NamedTempFile::new().unwrap();
        let file_path = temp_file.path().to_str().unwrap();
